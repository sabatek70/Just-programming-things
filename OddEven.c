...
{
    for(int input_start, input_limit, step, code = scanf("%d%d%d", &input_start, &input_limit, &step, printf("<start> <end> <step>\n")), start = input_start, limit = input_limit, a = printf("", (input_limit-input_start>0&&step<=0)?start=limit+1:0, (input_limit-input_start<0&&step>=0)?limit=start+1:0);(input_start<=input_limit)?(start<=limit):(limit<=start); printf("\n", (start%2==0)?printf(" : even", start+=step):printf(" : odd", start+=step))) printf("%d", start);
    return 0;
}
